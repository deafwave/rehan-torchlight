...
export const DEFAULT_SNAPSHOT: Snapshot = {
  ...
  rotation: { starters_per_cycle: 3, starter_weapon_pct: 100,
...
function weaponAvg(b: Record<string, number>): number {
  let phys = (b.weapon_phys_min + b.weapon_phys_max) / 2;
  phys *= 1 + b.gear_phys_pct / 100;
  const cold = (b.weapon_flat_cold_min + b.weapon_flat_cold_max) / 2;
  return phys + cold;
}
...
export function cycleDps(s: Snapshot) {
  ...
  const starter = expectedHit(s, r.starter_weapon_pct) * r.starters_per_cycle;
  const amp = 1 + r.combo_points * r.finisher_amp_pct / 100;
  const finisher = expectedHit(s) * amp * (1 + r.clones * r.clone_falloff);
