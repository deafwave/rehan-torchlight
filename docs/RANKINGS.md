...
```
coverage: 107 matched, 90 ignored, 8 UNMATCHED of 205 lines (96% handled)
```
...
Baseline: **26,136.9M DPS**, 0.68s combo cycle (3 starters + finisher). Crit position 69.35% / 588.5% (×1.405 crit-gated support), penetration 10%, Frostbite ×2.88, Paralysis ×1.15, attack speed +209%, 5 finisher clones, mitigation 0.80 × 0.91 (30% res − 10 pen; 30% armor reduction − 21 pen from the Perishing Inferno Flame Ring).

The 284.9M → 1,151.0M → 3,097.2M → 5,884.7M → 25,131.6M → 26,136.9M rebaselines are parser/model fixes, not build changes:
...
```
 ...
 +10.81%  +1 Combo Point consumed
  ...
  +3.44%  +10% Combo Finisher Amplification
  +3.24%  +10% attack speed
...
1. **Penetration — both kinds.** Mitigation is two ~30% layers: elemental res (10% pen in build → +12.5% DPS per further 10%) and armor elemental reduction (21% pen in build, all from the Perishing Inferno Flame Ring → +11.0% per further 10%). Both keep working past zero. Armor DMG Mitigation Penetration was previously mis-labelled a no-op here — the ring's 21% was being thrown away.
2. **Rank both prisms to 5.** Detonation and Legion are both rank 1; each rank adds +4% to the base +20% line, so rank 5 is +13.3% DPS per prism — the two cheapest multipliers on the sheet, beaten only by penetration.
3. **Combo Points consumed.** Finisher damage is `1 + points × amp` with amp already at 251.2%; going 8 → 9 points is +10.8% DPS. Anything granting +max Combo Points or more points consumed per finisher beats almost any damage roll.
...
5. **Crit chance and crit damage are nearly level now.** At 69.35% chance / 588.5% crit damage, +5% chance ≈ +5.6% vs +30% crit damage ≈ +4.7%. The pactspirit rating nodes (+177% between Red Umbrella and Azure Gunslinger) closed most of the gap that used to favour chance. Crit rolls still beat any "% increased damage" roll ~4:1.
6. **Attack speed** (+4.6% per 10%) still beats every "% increased damage" roll (+1.7% per 10%). The increased pile is at 400%+ — stop paying for %damage, %phys, %attack, %melee, %area, %cold rolls; they are the worst stats on this sheet.
...
- **Fervor rating sustained at 100** and **full Rage Infusion stacks / Berserk uptime** are inferred (mechanics.md); if uptime is worse, crit numbers drop and additional-pile points get slightly better.
...
- **Spectral Slash's support gems are now modeled at real levels** (mechanics.md#supports), including +10 Support Skill Levels → L30 reads and Legion's +1 finisher Clone vs boss. Thunder Spike's supports are deliberately excluded — it is mobility, not damage, in this build.
...
- **Cold Infiltration and Freeze uptime**: the slate applies it to *Frozen* enemies; boss Freeze uptime is assumed high. If bosses resist Freeze, drop `enemy_taken.cold_infiltration` and 10 pen from the snapshot (−~20% baseline, rankings order unchanged).
